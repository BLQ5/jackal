...
func TestMySQLStorage_ResourcesUpsert(t *testing.T) {
	j1, _ := jid.NewWithString("ortuman@jackal.im/yard", true)

	s, mock := newResourcesMock()

	mock.ExpectExec("INSERT INTO resources (.+) VALUES (.+) ON DUPLICATE KEY UPDATE allocation_id = \\?, priority = \\?, updated_at = NOW\\(\\)").
		WithArgs("a1234", "ortuman", "jackal.im", "yard", 8, "a1234", 8).
...
func TestMySQLStorage_ResourcesDelete(t *testing.T) {
	s, mock := newResourcesMock()
...
func TestMySQLStorage_ResourcesFetch(t *testing.T) {
