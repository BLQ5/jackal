...
func (s *Presences) FetchPresence(ctx context.Context, jid *jid.JID) (*model.ExtPresence, error) {
	...
	switch err {
	case nil:
		return scanExtendedPresence(rawXML, node, ver, featuresJSON)
	case sql.ErrNoRows:
		return nil, nil
	default:
		return nil, err
...
func (s *Presences) FetchPresencesMatchingJID(ctx context.Context, jid *jid.JID) ([]model.ExtPresence, error) {
	...
	for rows.Next() {
		...
		presenceCaps, err := scanExtendedPresence(rawXML, node, ver, featuresJSON)
		if err != nil {
			return nil, err
		}
...
func scanExtendedPresence(rawXML, node, ver, featuresJSON string) (*model.ExtPresence, error) {
